...
const Header = ({ variant = 'default', logoText = 'Holiday Deals', logoImage }) => {
    ...
    return (<header className={`fixed h-24 top-0 py-1 z-50 w-full transition-all ${sticky
      ...
      <div className='container mx-auto max-w-6xl flex items-center justify-between p-6'>
        ...
        <ul className='hidden lg:flex grow items-center justify-center gap-6'>
          {headerData.map((item, index) => (<li key={index}>
        ...
        <div className='flex items-center gap-4'>
          {/* Theme Toggle */}
          <button aria-label='Toggle theme' onClick={() => setTheme(theme === 'dark' ? 'light' : 'dark')} className='flex h-8 w-8 items-center justify-center text-body-color duration-300 dark:text-white'>
          ...
          {/* Sign In/Up Buttons */}
          <Link href='/signin' className='hidden lg:block bg-transparent border border-primary text-primary px-4 py-2 rounded-lg hover:bg-blue-600 hover:text-white'>
            Sign In
          </Link>
          <Link href='/signup' className='hidden lg:block bg-primary text-white px-4 py-2 rounded-lg hover:bg-blue-700'>
            Sign Up
          </Link>
      ...
      {/* Mobile Menu */}
      <div ref={mobileMenuRef} className={`lg:hidden fixed top-0 right-0 h-full w-full bg-white dark:bg-darkmode shadow-lg transform transition-transform duration-300 max-w-xs ${navbarOpen ? 'translate-x-0' : 'translate-x-full'} z-50`}>
        ...
        <nav className='flex flex-col items-start p-4'>
          {headerData.map((item, index) => (<Link key={index} href={item.path} className={`text-base font-medium py-2 w-full ${pathUrl === item.path
                ? 'text-primary'
                : 'text-midnight_text dark:text-white hover:text-primary'}`} onClick={() => setNavbarOpen(false)}>
              {item.title}
            </Link>))}
          <div className='mt-4 flex flex-col gap-4 w-full'>
            <Link href='/signin' className='bg-transparent border border-primary text-primary px-4 py-2 rounded-lg hover:bg-blue-600 hover:text-white text-center' onClick={() => setNavbarOpen(false)}>
              Sign In
            </Link>
            <Link href='/signup' className='bg-primary text-white px-4 py-2 rounded-lg hover:bg-blue-700 text-center' onClick={() => setNavbarOpen(false)}>
              Sign Up
            </Link>
          </div>
        </nav>
