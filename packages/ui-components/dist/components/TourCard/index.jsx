...
const TourCard = ({ tour, variant = 'default', showPrice = true, className = '' }) => {
    ...
    const renderStars = (rating) => {
        ...
        {Array(fullStars)
                .fill(0)
                .map((_, index) => (<svg key={index} className="w-5 h-5 text-yellow-400" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2l2.39 7.27L22 9.27l-5.5 4.21L17.18 22 12 17.27 6.82 22l.68-8.52L2 9.27l7.61-1L12 2z"/>
            </svg>))}
        {/* Half star */}
        {halfStar && (<svg className="w-5 h-5" fill="currentColor" viewBox="0 0 40 40" xmlns="http://www.w3.org/2000/svg">
            <defs>
            ...
            <path d="M12 2l2.39 7.27L22 9.27l-5.5 4.21L17.18 22 12 17.27 6.82 22l.68-8.52L2 9.27l7.61-1L12 2z" fill="url(#halfStarGradient)"/>
          </svg>)}
        {/* Empty stars */}
        {Array(emptyStars)
                .fill(0)
                .map((_, index) => (<svg key={index} className="w-5 h-5 text-gray-300" fill="currentColor" viewBox="0 0 24 24">
              <path d="M12 2l2.39 7.27L22 9.27l-5.5 4.21L17.18 22 12 17.27 6.82 22l.68-8.52L2 9.27l7.61-1L12 2z"/>
            </svg>))}
      </div>);
    };
    ...
    return (<Link href={`/tours/${tour.slug}`} className={`group ${className}`}>
      <div className={`rounded-3xl overflow-hidden shadow-lg hover:shadow-xl transition-all duration-300 ${currentVariant.card}`}>
        <div className="relative overflow-hidden">
          <Image src={tour.image} alt={tour.title} width={400} height={300} className="w-full h-64 object-cover group-hover:scale-110 transition-transform duration-500"/>
          {showPrice && (<div className={`absolute top-3 right-3 rounded-full py-1 px-4 ${currentVariant.price}`}>
              <p className="text-white font-medium text-sm">
                {tour.pricing.currency}{tour.pricing.basePrice}
              </p>
            </div>)}
          {tour.featured && (<div className="absolute top-3 left-3 rounded-full py-1 px-3 bg-red-500">
